...
async function add( Text, Post_id, Owner_id){
    const sql = `INSERT INTO Comments (created_at, Text, Post_id, Owner_id) VALUES ? ;`;
...
const search = async q => await mysql.query(`SELECT id, Text, Post_id FROM Comments WHERE Text LIKE ? ; `, [`%${q}%`]);
