...
async function getForPost(post_id){
    const sql = `SELECT P.*, FirstName, LastName FROM Reactions P Join Users U ON P.Owner_id = U.id WHERE P.Post_id = ?`
...
async function add( Emoji = Emojis.LOVE, Post_id, Owner_id){
    const sql = `INSERT INTO Reactions (created_at, Emoji, Post_id, Owner_id) VALUES ? ;`;
